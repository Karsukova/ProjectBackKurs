...
public class EventServiceImpl implements EventService {
    ...
    public List<EventDTO> findAll() {
        return eventRepo.findAll()
                ...
                .map(event -> new EventDTO(event.getId(), event.getTitle(),
                        event.getStart(), event.getIsplus(), event.getSum()))
    ...
    public List<EventDTO> findAllByUser(User user) {
        return eventRepo.findAllByUser(user)
                .stream()
                .map(event -> new EventDTO(event.getId(), event.getTitle(), event.getStart(), event.getIsplus(), event.getSum()))
                .collect(Collectors.toList());
    }
